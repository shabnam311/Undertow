...
const TOP_LEVEL_SCHEMA_COMPOSITION_KEYS = [
	"oneOf",
	"anyOf",
	"allOf"
];
...
*/
function getTopLevelSchemaCompositionKeys(tool) {
	if (typeof tool.input_schema !== "object" || tool.input_schema === null) return [];
	return TOP_LEVEL_SCHEMA_COMPOSITION_KEYS.filter((key) => Object.prototype.hasOwnProperty.call(tool.input_schema, key));
}
...
const AnthropicToolExtrasSchema = zod_v4.object({
	...
	strict: zod_v4.boolean().optional()
});
/**
...
exports.getTopLevelSchemaCompositionKeys = getTopLevelSchemaCompositionKeys;
